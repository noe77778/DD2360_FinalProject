...
typedef struct record
{
  ...
  __nv_bfloat16 distance;
} Record;

int loadData(char *filename,std::vector<Record> &records,std::vector<LatLong> &locations);
void findLowest(std::vector<Record> &records,__nv_bfloat16 *distances,int numRecords,int topN);
void printUsage();
int parseCommandline(int argc, char *argv[], char* filename,int *r,__nv_bfloat16 *lat,__nv_bfloat16 *lng,
...
*/
__global__ void euclid(LatLong *d_locations, __nv_bfloat16 *d_distances, int numRecords, __nv_bfloat16 lat, __nv_bfloat16 lng)
{
  ...
  if (globalId < numRecords) {
        __nv_bfloat16 *dist=d_distances+globalId;
        *dist = (__nv_bfloat16)sqrt((Datatype)((lat-latLong->lat)*(lat-latLong->lat)+(lng-latLong->lng)*(lng-latLong->lng)));
  }
}
...
int main(int argc, char* argv[])
{
  ...
  __nv_bfloat16 *distances;
  //Pointers to device memory
  LatLong *d_locations;
  __nv_bfloat16 *d_distances;
  ...
  */
  distances = (__nv_bfloat16 *)malloc(sizeof(__nv_bfloat16) * numRecords);
  cudaMalloc((void **) &d_locations,sizeof(LatLong) * numRecords);
  cudaMalloc((void **) &d_distances,sizeof(__nv_bfloat16) * numRecords);
    ...
    */
    ...
    cudaMemcpy( distances, d_distances, sizeof(__nv_bfloat16)*numRecords, cudaMemcpyDeviceToHost );
    ////////////////////////////////////////////////////////////////////////////////
  ...
  // find the resultsCount least distances
    ...
    for(i=0;i<resultsCount;i++) {//for(i=0;i<resultsCount;i++)
      printf("%s --> Distance=%f\n",records[i].recString,(__bfloat162float)(records[i].distance));
    }
    free(distances);
    //Free memory
...
void findLowest(std::vector<Record> &records,__nv_bfloat16 *distances,int numRecords,int topN){
  ...
  int minLoc;
  Record *tempRec;
  __nv_bfloat16 tempDist;

  for(i=0;i<topN;i++) {
    minLoc = i;
    for(j=i;j<numRecords;j++) {
      val = distances[j];
      if ((__bfloat162float)(val) < (__bfloat162float)(distances[minLoc])) minLoc = j;
...
void printUsage(){
  ...
  printf("\n");
  printf("nearestNeighbor [filename] -r [int] -lat [__nv_bfloat16] -lng [__nv_bfloat16] [-hqt] [-p [int] -d [int]]\n");
  printf("\n");
  printf("example:\n");
